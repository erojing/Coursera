...
FROM
  fireIncidents
WHERE
  LEFT(`Incident Date`, 5) = '04/04'
OR
  LEFT(`Incident Date`, 5) = '09/27'
...
SELECT
  `Incident Date`,
  COUNT(DISTINCT `Incident Number`) AS Incident_count
FROM
  fireIncidents
WHERE
  `Incident Date` = '04/04/2016'
...
SELECT
  COUNT(fireIncidents.`Incident Number`) AS total_incident_counts
FROM
  fireIncidents
INNER JOIN
  fireCalls
